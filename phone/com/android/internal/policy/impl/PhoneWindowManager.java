...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    boolean mShouldTranslateCallOnKeyUp;
    ...
    Runnable mCallLongPress = new Runnable() {
        public void run() {
            mShouldTranslateCallOnKeyUp = false;
            performHapticFeedbackLw(null, HapticFeedbackConstants.LONG_PRESS, false);
            sendCloseSystemWindows(SYSTEM_DIALOG_REASON_GLOBAL_ACTIONS);
	    startCallActivity();
        }
    ...
    public int interceptKeyTq(RawInputEvent event, boolean screenIsOn) {
        ...
        if (type == RawInputEvent.EV_KEY) {
	     if (code == KeyEvent.KEYCODE_ENDCALL) {
	        /* ONE key translation hack */
	        ITelephony phoneServ = getPhoneInterface();
	        boolean isRinging = false;
	
	         try {
	             if (phoneServ != null && phoneServ.isRinging()) {
	                isRinging = true;
	            }
	         } catch(RemoteException ex) {}
	
	        if (!isInCall() && !isRinging) {
	            result |= ACTION_TRANSLATE_KEY;
	        } else if (down) {
                    ...
                    if (handled) {
                       // If we hung up a call, stop processing the
                       // key here...
                       result &= ~ACTION_PASS_TO_USER;
                    }
                    /* ONE HACK: we're done 
		...
		/* The ONE has a power key, use it! */
                if (down) {
  		    if (!screenIsOn) {
                        mShouldTurnOffOnKeyUp = false;
                    } else {
                    ...
                    if (mShouldTurnOffOnKeyUp) {
                        mShouldTurnOffOnKeyUp = false;
                        boolean gohome = (mEndcallBehavior & Settings.System.END_BUTTON_BEHAVIOR_HOME) != 0;
                        boolean sleeps = (mEndcallBehavior & Settings.System.END_BUTTON_BEHAVIOR_SLEEP) != 0;
                        if (keyguardOn()
                                || (sleeps && !gohome)
                                || (gohome && !goHome() && sleeps)) {
                            // they must already be on the keyguad or home screen,
				...
				// ONE hack - Hook a long press event to 
				// see if we want to raise the call menu
                        	mShouldTranslateCallOnKeyUp = true;
 				if (screenIsOn && !keyguardOn()) {
				    /* Only do longpress with screen On */
			    ...
			    /* ONE key translation hack */
                            ...
                            if (!mShouldTranslateCallOnKeyUp) {
                                result &= ~ACTION_PASS_TO_USER;
		            } else {
                                mShouldTranslateCallOnKeyUp = false;
		    		result |= ACTION_TRANSLATE_KEY;
